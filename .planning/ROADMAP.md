...
**In scope**: Truthful backend/frontend/integration/browser commands; phase-entry/TODO/trace validators; machine-readable obligation queries; UI manifest schema; route/page/DOM/test-ID/Playwright bidirectional drift checks; dynamic-row selector law; evidence metadata.
**Out of scope**: Business behavior and visual design decisions.
**Depends on**: Nothing.
...
**Entry gate**: Phase 1 has no dependency artifacts and does not invoke any validator that Phase 1 will later create. Required inputs are `docs/PRD.md`, `.planning/PRD-OBLIGATIONS.md`, and `.planning/phases/01-engineering-verification-foundation/`. After the independent reviewer writes unique criterion rows using the exact `Criterion ID | Verdict | Evidence | Command or inspection rule` columns plus a final `## Verdict`, run `/usr/bin/env ruby .planning/tools/bootstrap-phase-01.rb --phase-dir .planning/phases/01-engineering-verification-foundation --catalog .planning/PRD-OBLIGATIONS.md`. That existing Ruby bootstrap extracts every catalog row whose owner is `engineering-verification-foundation`, requires each exact obligation ID in `01-SPEC.md`, `TODO.md`, and `TEST-MATRIX.md`, requires `INTENT.md`, `DESIGN.md`, `ITERATIONS.md`, `DECISIONS.md`, `ENTRY-REVIEW.md`, `EVIDENCE/`, and at least one `01-*-PLAN.md`, and requires every `<task>` in every plan to contain nonempty `<files>`, `<action>`, `<verify>`, and `<done>`. The bootstrap exits nonzero for a missing artifact, absent owned-ID trace, malformed/duplicate/empty criterion row, empty required task field, missing final PASS, or any BLOCKER. The independent reviewer must classify a contradiction or non-runnable verification as BLOCKER, which the bootstrap then rejects. That reviewer applies the bounded revision cycle from `EXECUTION-STANDARD.md`: at most three review attempts per cycle; a non-decreasing finding count or exhausted cycle escalates with scoped TODOs still open, and only new developer decisions or evidence can start a new cycle.
...
**Plans**: At least one executable `01-*-PLAN.md`; exact plan count is derived from behavior slicing and is checked by the bootstrap rather than estimated.
...
**In scope**: Composition only: verify all 108 groups/all 522 atomic obligations; HTTP and CMPP downstream; real HTTP and CMPP upstream; compliance, routing, receipts, Webhook/CMPP reports, billing, lifecycle, UI UAT, archive, remote delivery evidence; release TODO query.
**Out of scope**: New features, new UI, or remediation hidden outside owning phases.
**Depends on**: Phases 1-55.
**Requirements**: Atomic PRD DoD and cross-protocol composition obligations.
...
3. GSD and final Claude reviews are clear, repository TODO query is empty, and the atomic release commit is visible on the configured GitHub remote.
