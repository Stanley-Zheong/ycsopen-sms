...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| OBL-ISSUE-58-ADMIN-TENANTS | PROJECT-UI-CONTRACT | issue-58-admin-tenants | T-ISSUE-58-QUERY-PANEL:playwright | pw-issue-58-admin-tenants | admin-tenants `/admin/tenants` | query-panel | C-ISSUE-58-ADMIN-TENANTS | Chromium opens all three desktop fields on one row without a redundant toggle, verifies every label/control pair, applies all three client-side tenant filters, advances to page two, then reset clears every control, returns to page one and restores the initial rows. | Use the exact 3-case command in `EVIDENCE/playwright-query-panel-report.json` with `--grep 'pw-issue-58-admin-tenants'` for an isolated replay. | Historical mocked browser PASS is checksum-bound to the pre-Issue-63 layout. Current-layout replay remains required together with real-service acceptance. |
