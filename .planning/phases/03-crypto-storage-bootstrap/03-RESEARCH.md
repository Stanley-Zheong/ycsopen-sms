...
The V1 schema also contains one **schema-only migratable digest surface**: `third_party_risk_check_logs.mobile_hash` is `CHAR(64) NOT NULL` with `idx_mobile_hash`, and the PRD classifies the table as third-party risk-call history containing the protected called number. The current repository has no Entity, Repository, SQL reader or SQL writer for this table; `ThirdPartyBlacklistClient` is still a non-persisting placeholder. It therefore must not be represented as a current executable Java surface, but it remains in the exact-five legacy blind-index migration/scrub union because deployed databases may contain historical raw SHA-256 rows. Plans 01/12 classify it `MIGRATABLE_SCHEMA_ONLY`; plans 13/14 own migration. The deployed-writer fence must prove that no unknown external writer still writes the table, otherwise its target checkpoint and OBL-001/004 evidence remain blocked.
