...
- [ ] Corrective commit is pushed to PR 15 and synthetic-merge backend, real-service, and Web checks pass — Evidence: the historical Phase 03 tag remains immutable and is superseded by the corrective PR head.
...
- [ ] CI replay CR-10: MinIO validation distinguishes the immutable manifest/repository digest from the OCI config/image digest on classic Linux Docker — Evidence: 15 service-contract cases / 70 assertions PASS; independent reviewer and Claude report BLOCKER/HIGH 0/0; pending synthetic-merge replay.
- [ ] CI replay CR-11: a clean generated-output tree supplies the artifact scanner with a validated current-run real proof before the combined leak scan — Evidence: empty-input scanner fixtures 24/24 PASS; clean local real leak suite 1/1 PASS with zero skip and no retained temporary input; independent and Claude reviews BLOCKER/HIGH 0/0; pending synthetic-merge execution of all seven real suites with zero skips.
...
- [ ] Pull-request CI tests the synthetic merge tree — Evidence: pending workflow correction and PR run.
- [ ] Phase 03 real-service evidence is generated in CI and no `core/target` result is tracked — Evidence: pending workflow correction and repository query.
- [ ] CI runs frontend install, unit tests, and production build — Evidence: pending workflow correction and PR run.
