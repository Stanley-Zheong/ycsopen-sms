...
These corrections reopen only the affected TODOs and the final review/delivery rows. Aggregate review repeats until no scoped BLOCKER/HIGH remains; Claude receives the final bounded remediation patch rather than the unrelated Phase 1-3 cumulative branch diff. The CI image-preparation correction is a delivery-environment prerequisite and is accepted only by a fresh synthetic-merge real-integration run with all seven named suites executed and zero skips.
