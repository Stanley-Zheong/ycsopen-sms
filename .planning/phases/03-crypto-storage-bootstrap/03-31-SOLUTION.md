...
9. **A clean leak-scan run bootstraps from a real current-run report.** Removing the tracked `core/target/**` files exposed that `Phase03LeakScanIntegrationTest` invoked the artifact scanner before the current Maven run had produced any report under `core/target/phase03`; the scanner correctly rejected `REPORTS_INPUTS_EMPTY`. Do not weaken that fail-closed rule and do not restore committed/generated evidence. The leak integration test must first capture the already executed real PKCS#11 child proof's sanitized PASS output and validate it through the producer test's single shared closed-format verifier; do not duplicate the output's numeric contract in the leak test. `startAll()` already creates and validates its run-owned service handoff below `core/target/phase03/services`, so the leak test must not create this directory through an unchecked path. It requires the existing root's resolved path to equal the expected repository-owned path before writing a run-owned temporary `.txt`; absence or a symlink substitution fails closed. Invoke the unchanged artifact scanner and delete the temporary input in `finally`. The resulting artifact report then participates in the existing five-surface Java leak scan. A clean synthetic-merge runner must execute all seven named suites with zero skips.
10. **The active validation contract must describe the final executable truth.** `03-VALIDATION.md` is an active Phase 03 contract, not an archived planning snapshot. It therefore cannot remain `draft`, retain `pending` plan rows, name the superseded `Phase03FullFaultIntegrationTest`, prohibit the CI job's explicit digest-pinned service-image preparation, or leave Wave 0/sign-off checkboxes open after closure is claimed elsewhere. Reconcile each status and checklist item against repository or run evidence; replace the monolithic-fault reference with the seven-suite real boundary recorded by the final workflow and decision log; distinguish forbidden implicit/moving test downloads from explicit CI preparation of immutable images and fixed system packages. Close CR-12 only after planning-validator checks, an independent consistency review, and a fresh latest-head PR CI run pass. Historical subject seals remain historical and are not regenerated merely because this active status document changes.
11. **Final verification rows use current successors, not deleted historical tests.** A plan summary may truthfully record the command that passed when that plan completed, but the active `03-VALIDATION.md` final map must remain executable on the current tree. Plan 26 intentionally removed the direct-key `FieldEncryptor` and `FieldEncryptorTest`; therefore Plan 24's final verification is the retained `ProtectedFieldCodecTest` plus explicit absence checks for both retired files. Audit every Java `*Test` token in the active map against `core/src/test` so the fix is not a one-name substitution.
12. **CR-12 closes through two non-self-referential commits.** The contract-correction commit keeps `03-VALIDATION.md` at `status: verifying`, `nyquist_compliant: false`, leaves its external-review/contract-head-CI sign-offs unchecked, and keeps CR-12 open everywhere. Independent and Claude reviewers inspect that exact correction, then PR CI tests its pushed commit. Only after those external facts exist may a closure-record commit cite the correction SHA/run, set validation `complete`/Nyquist true, close CR-12 and advance the roadmap/state. The final metadata-only commit must pass its own PR checks; a failure reopens a new solution-first corrective row rather than rewriting history.
13. **Evidence state must expose provenance.** The 30-row map may say `verified` for current executable outcomes, but Plans 02, 22 and 23 must explicitly identify their exact-four/root-registry evidence as sealed-subject evidence and must not imply that Plan 31 regenerated ignored/tracked outputs. Current Plan 31 acceptance remains the direct default/seven-suite/Web/static/review boundary. Historical and current evidence may coexist only with visibly distinct labels.
14. **Execution-shape overrides cite their real records.** The seven-suite replacement for the absent monolithic fault class must cite `03-VERIFICATION.md`'s `DR-P03-012` entry and the matching `03-DECISIONS.md` plan override, not an unexplained identifier.
15. **Do not weaken prerequisite or method evidence while reconciling prose.** Retain the fail-closed rule that absent real prerequisites stay open and mocks cannot close them. Where a plan row names a method-level check, either keep it as a currently executable focused command or identify the current suite evidence that includes it; the two-test real migration suite is the current proof for `appliesV1200WithoutChangingV1`.
16. **Current closure prose uses the physical-TODO/PR-check contract.** Historical iteration rows may retain the earlier annotated-tag experiments as history, but active footers, summaries and state must not reinstate that superseded external tag as a current completion gate. Current completion requires the scoped TODO to become physically empty and the latest closure head's live PR checks to pass.
...
The obsolete delivery tag remains historical and is not moved. Phase 03 closes on the correction commits pushed to PR #15, a successful latest-head synthetic-merge backend/real-service/Web CI run, blocking-free aggregate independent and Claude reviews, an internally consistent active validation contract, and a physically empty TODO query.
