...
async function mockMessageOperationsApis(page: Page, fixtures: { sendRows?: typeof sends; errorRows?: typeof errors } = {}) {
  ...
  await page.route('**/api/v1/console/message-operations/sends?**', (route: Route) => route.fulfill({ json: apiResponse(fixtures.sendRows ?? sends) }));
  await page.route('**/api/v1/console/message-operations/receipts?**', (route: Route) => route.fulfill({ json: apiResponse(receipts) }));
  await page.route('**/api/v1/console/message-operations/errors?**', (route: Route) => route.fulfill({ json: apiResponse(fixtures.errorRows ?? errors) }));
...
test.describe('Phase 27 message receipt error operations', () => {
  ...
  test('pw-p27-error-page C-P27-ERROR-PAGE OBL-F-7-6-A pw-p27-error-bulk-retry C-P27-ERROR-BULK-RETRY OBL-F-7-6-B', async ({ page }) => {
    await mockMessageOperationsApis(page, { sendRows: [...otherFailedSend, ...sends], errorRows: [...otherError, ...errors] });
    await loginAs(page, 'OPERATOR');
    await page.goto('/admin/error/details');
    await expect(page.getByTestId('admin-message-receipt-error-details-page')).toBeVisible();
    const e42Row = page.getByTestId('admin-message-receipt-error-details-row').filter({ hasText: 'E42' });
    await expect(page.getByTestId('admin-message-receipt-error-details-row')).toHaveCount(2);
    await expect(e42Row).toContainText('E42');
    await e42Row.getByTestId('admin-message-receipt-error-details-bulk-retry').click();
