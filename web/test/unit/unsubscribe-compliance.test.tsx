...
describe('Phase 33 unsubscribe compliance UI', () => {
  ...
  it('shows admin keyword evidence statistics and alert actions', async () => {
    ...
    vi.mocked(api.listAdminUnsubscribes).mockClear();
    fireEvent.change(screen.getByTestId('admin-unsubscribe-compliance-filter-tenant'), { target: { value: '7' } });
    expect(api.listAdminUnsubscribes).not.toHaveBeenCalled();
    fireEvent.click(within(panel).getByTestId('query-submit'));
    await waitFor(() => expect(api.listAdminUnsubscribes).toHaveBeenCalledWith(expect.objectContaining({ tenantId: '7' })));
    fireEvent.click(within(panel).getByTestId('query-reset'));
    expect(screen.getByTestId('admin-unsubscribe-compliance-filter-tenant')).toHaveValue('');
    await waitFor(() => expect(api.listAdminUnsubscribes).toHaveBeenCalledWith({ tenantId: '', mobile: '', keyword: '', outcome: '', signatureId: '', productCode: '', notificationState: '' }));
    expect(within(panel).getByTestId('query-result-table')).toHaveTextContent('138****8000');
  ...
  it('shows tenant scoped evidence keyword management and export request', async () => {
    ...
    expect(within(panel).getByTestId('query-panel-fields')).not.toBeVisible();
    fireEvent.click(within(panel).getByTestId('query-panel-toggle'));
    vi.mocked(api.listTenantUnsubscribes).mockClear();
    fireEvent.change(screen.getByTestId('tenant-unsubscribe-compliance-filter-mobile'), { target: { value: '13800138000' } });
    expect(api.listTenantUnsubscribes).not.toHaveBeenCalled();
    fireEvent.click(within(panel).getByTestId('query-submit'));
    await waitFor(() => expect(api.listTenantUnsubscribes).toHaveBeenCalledWith(expect.objectContaining({ mobile: '13800138000' })));
    fireEvent.click(within(panel).getByTestId('query-reset'));
    expect(screen.getByTestId('tenant-unsubscribe-compliance-filter-mobile')).toHaveValue('');
    await waitFor(() => expect(api.listTenantUnsubscribes).toHaveBeenCalledWith({ mobile: '', keyword: '', outcome: '', signatureId: '', productCode: '', notificationState: '' }));
    expect(within(panel).getByTestId('query-result-table')).toHaveTextContent('TD');
