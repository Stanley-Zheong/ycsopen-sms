...
export default function TenantBulkSendPage() {
  ...
  const previewMutation = useMutation({
    ...
    onSuccess: (result) => {
      setPreview(result);
      setMessage(`校验完成：有效 ${result.valid}，无效 ${result.invalid}`);
      setError('');
    },
    onError: (failure) => {
      setError(mutationErrorMessage(failure, '批量校验失败'));
      setMessage('');
    },
  });
  const createMutation = useMutation({
    ...
    onError: (failure) => {
      setError(mutationErrorMessage(failure, '批任务创建失败'));
      setMessage('');
    },
  });
  ...
  return (
    <section className="bulk-scheduled-page" data-testid="tenant-bulk-scheduled-bulk-send-page">
      ...
      <header><h1>批量发送</h1><p className="page-description">CSV/Excel 导入前先校验文件、号码、变量和重复行，再创建批任务。</p></header>
      ...
      <form className="card bulk-form" data-testid="tenant-bulk-scheduled-bulk-send-form" onSubmit={submit}>
        <label>批次流水<input data-testid="tenant-bulk-scheduled-bulk-send-batch-key" value={batchKey} onChange={(event) => setBatchKey(event.target.value)} /></label>
        <label>任务名称<input data-testid="tenant-bulk-scheduled-bulk-send-task-name" value={taskName} onChange={(event) => setTaskName(event.target.value)} /></label>
        ...
        <label className="bulk-wide">导入内容<textarea data-testid="tenant-bulk-scheduled-bulk-send-upload" value={rowsText} onChange={(event) => setRowsText(event.target.value)} /></label>
      ...
      <section className="card" data-testid="tenant-bulk-scheduled-bulk-send-validation-results">
        <h2>校验结果</h2>
