...
export default function TemplateLifecyclePage() {
  ...
  const submit = useMutation({
    ...
    onError: (failure) => setError(mutationErrorMessage(failure, '模板申请失败')),
  });
  ...
  return (
    <section data-testid="tenant-template-lifecycle-templates-page">
      ...
      <form className="card template-lifecycle-form" onSubmit={submitForm}>
        <label>模板名称
          <input data-testid="tenant-template-lifecycle-templates-form-name" value={form.templateName} onChange={(event) => update('templateName', event.target.value)} />
        </label>
        <label>模板内容
          <textarea data-testid="tenant-template-lifecycle-templates-form-content" value={form.content} onChange={(event) => update('content', event.target.value)} />
        </label>
        <label>模板类型
          <select data-testid="tenant-template-lifecycle-templates-form-type" value={form.templateType} onChange={(event) => update('templateType', event.target.value)}>
        ...
        <label>绑定签名ID
          <input data-testid="tenant-template-lifecycle-templates-form-signature" value={form.signatureId || ''} onChange={(event) => update('signatureId', Number(event.target.value))} />
        </label>
        <label>参数规则
          <input data-testid="tenant-template-lifecycle-templates-form-param-rule" value={form.paramCheckRule} onChange={(event) => update('paramCheckRule', event.target.value)} />
        </label>
        <label>申请说明
          <input value={form.description} onChange={(event) => update('description', event.target.value)} />
        </label>
        <button type="submit" data-testid="tenant-template-lifecycle-templates-submit" disabled={submit.isPending}>提交模板申请</button>
      </form>
