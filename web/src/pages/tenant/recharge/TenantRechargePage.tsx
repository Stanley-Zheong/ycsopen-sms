...
export default function TenantRechargePage() {
  ...
  const mutation = useMutation({
    ...
    onError: (failure) => {
      setError(mutationErrorMessage(failure, '充值申请提交失败'));
      setMessage('');
    },
  });
  ...
  return (
    <section className="tenant-recharge-page" data-testid="tenant-recharge-operations-recharge-page">
      ...
      {error && <p role="alert" className="tenant-recharge-alert error" data-testid="tenant-recharge-operations-recharge-error">{error}</p>}

      <section className="card" data-testid="tenant-recharge-operations-recharge-form">
        <h2>充值申请</h2>
        <div className="tenant-recharge-form">
          <label>金额（厘）<input data-testid="tenant-recharge-operations-recharge-amount" type="number" min="1" value={amountMil} onChange={(event) => setAmountMil(event.target.value)} /></label>
          <label>充值方式
            <select data-testid="tenant-recharge-operations-recharge-method" value={rechargeMethod} onChange={(event) => setRechargeMethod(event.target.value)}>
              <option value="BANK_TRANSFER">银行转账</option>
              <option value="ALIPAY">支付宝</option>
              <option value="WECHAT">微信</option>
              <option value="OFFLINE">线下确认</option>
            </select>
          </label>
          <label>交易号<input data-testid="tenant-recharge-operations-recharge-transaction" value={transactionRef} onChange={(event) => setTransactionRef(event.target.value)} /></label>
          <label>凭证<input data-testid="tenant-recharge-operations-recharge-evidence" value={evidenceText} onChange={(event) => setEvidenceText(event.target.value)} /></label>
          <button type="button" data-testid="tenant-recharge-operations-recharge-submit" onClick={() => mutation.mutate()}>提交充值申请</button>
        </div>
      </section>
