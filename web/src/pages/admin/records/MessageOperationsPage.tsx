...
const EMPTY_FILTER: OperationFilter = { tenantId: '', messageId: '', status: '', errorCode: '' };
...
export default function MessageOperationsPage({ initialSection = 'submissions' }: { initialSection?: Section }) {
  ...
  const [draftFilter, setDraftFilter] = useState<OperationFilter>(EMPTY_FILTER);
  const [appliedFilter, setAppliedFilter] = useState<OperationFilter>(EMPTY_FILTER);
          ...
          <>
            ...
            {section === 'submissions' && (
              <section className="card" data-testid="admin-message-receipt-submission-details-trace">
                <h2>提交详情链路</h2>
                {submissions.isLoading && <p>正在加载提交详情…</p>}
                {submissions.isError && <p role="alert">提交详情加载失败。</p>}
                {!submissions.isLoading && !submissions.isError && (submissions.data ?? []).length === 0 && <p>暂无提交详情。</p>}
            ...
            {section === 'sends' && (
              <section className="card">
                <h2>发送详情</h2>
                {sends.isLoading && <p>正在加载发送详情…</p>}
                {sends.isError && <p role="alert">发送详情加载失败。</p>}
                {!sends.isLoading && !sends.isError && (sends.data ?? []).length === 0 && <p>暂无发送详情。</p>}
            ...
            {section === 'receipts' && (
              <section className="card">
                <h2>回执详情</h2>
                {receipts.isLoading && <p>正在加载回执详情…</p>}
                {receipts.isError && <p role="alert">回执详情加载失败。</p>}
                {!receipts.isLoading && !receipts.isError && (receipts.data ?? []).length === 0 && <p>暂无回执详情。</p>}
            ...
            {section === 'errors' && (
              <section className="card">
                ...
                {errors.isLoading && <p>正在加载错误详情…</p>}
                {errors.isError && <p role="alert">错误详情加载失败。</p>}
                {!errors.isLoading && !errors.isError && (errors.data ?? []).length === 0 && <p>暂无错误详情。</p>}
